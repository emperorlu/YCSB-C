...
void HistogramStatClear(HistogramStat *stat) {
    ...
    stat->num_.store(0);
    stat->sum_.store(0);
    stat->sum_squares_.store(0);
    for(uint64_t i = 0; i < stat->num_buckets_; i++){
        stat->buckets_[i].store(0);
    }
}
...
void HistogramStatAdd(HistogramStat *stat, uint64_t value) {
    ...
    stat->buckets_[index].fetch_add(1, std::memory_order_relaxed);

    uint64_t old_min = stat->min_.load(std::memory_order_relaxed);
    if (value < old_min) {
        stat->min_.compare_exchange_weak(old_min, value, std::memory_order_relaxed);
    }

    uint64_t old_max = stat->max_.load(std::memory_order_relaxed);
    if (value > old_max) {
        stat->max_.compare_exchange_weak(old_max, value, std::memory_order_relaxed);
    }

    stat->num_.fetch_add(1, std::memory_order_relaxed);
    stat->sum_.fetch_add(value, std::memory_order_relaxed);
    stat->sum_squares_.fetch_add(value * value, std::memory_order_relaxed);
}
...
void HistogramStatMerge(HistogramStat *stat, HistogramStat *other) {
    ...
    uint64_t old_min = stat->min_.load(std::memory_order_relaxed);
    uint64_t other_min = other->min_.load(memory_order_relaxed);
    // while (other_min < old_min &&
    //         atomic_compare_exchange_weak(&stat->min_, &old_min, other_min)) {}
    if(other_min < old_min) stat->min_.compare_exchange_weak(old_min, other_min, memory_order_relaxed);

    uint64_t old_max = stat->max_.load(memory_order_relaxed);
    uint64_t other_max = other->max_.load(memory_order_relaxed);
    // while (other_max > old_max &&
    //         atomic_compare_exchange_weak(&stat->max_, &old_max, other_max)) {}
    if(other_max > old_max) stat->max_.compare_exchange_weak(old_max, other_max, memory_order_relaxed);

    uint64_t other_num = other->num_.load(memory_order_relaxed);
    stat->num_.fetch_add(other_num, memory_order_relaxed);

    uint64_t other_sum = other->sum_.load(memory_order_relaxed);
    stat->sum_.fetch_add(other_sum, memory_order_relaxed);
    
    uint64_t other_sum_squares = other->sum_squares_.load(memory_order_relaxed);
    stat->sum_squares_.fetch_add(other_sum_squares, memory_order_relaxed);

    for (unsigned int b = 0; b < stat->num_buckets_; b++) {
        uint64_t other_bucket = other->buckets_[b].load(memory_order_relaxed);
        stat->buckets_[b].fetch_add(other_bucket, memory_order_relaxed);
    }
}

double HistogramStatPercentile(HistogramStat *stat, double p) {
    uint64_t num = stat->num_.load(memory_order_relaxed);
    double threshold = num * (p / 100.0);
    uint64_t cumulative_sum = 0;
    for (unsigned int b = 0; b < stat->num_buckets_; b++) {
        uint64_t bucket_value = stat->buckets_[b].load(memory_order_relaxed);
        ...
        if (cumulative_sum >= threshold) {
            ...
            uint64_t cur_min = stat->min_.load(memory_order_relaxed);
            uint64_t cur_max = stat->max_.load(memory_order_relaxed);
            if (r < cur_min) r = (double)cur_min;
            if (r > cur_max) r = (double)cur_max;
            return r;
        }
    }
    double max = (double)stat->max_.load(memory_order_relaxed);
    return max;
}
...
double HistogramStatAverage(HistogramStat *stat) {
    uint64_t cur_num = stat->num_.load(memory_order_relaxed);
    uint64_t cur_sum = stat->sum_.load(memory_order_relaxed);
...
double HistogramStatStandardDeviation(HistogramStat *stat) {
    uint64_t cur_num = stat->num_.load(memory_order_relaxed);
    uint64_t cur_sum = stat->sum_.load(memory_order_relaxed);
    uint64_t cur_sum_squares = stat->sum_squares_.load(memory_order_relaxed);
    if (cur_num == 0) return 0;
    double variance =
...
char* HistogramStatToString(HistogramStat *stat) {
    ...
    uint64_t min = stat->min_.load(memory_order_relaxed);
    uint64_t max = stat->max_.load(memory_order_relaxed);
    snprintf(buf, sizeof(buf),
